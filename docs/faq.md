...
flowchart into a frontier model's prompt every turn, `subterranean` generates
...
[Modal](cloud.md) (`modal run -m subterranean.cloud.modal_app::reproduce_travel`).
...
Yes. `subterranean compile graph.py --out build/mine` imports a `StateGraph` and
...
Run `subterranean eval` and compare `eval_report.json` to the per-criterion
